...
//             <Link 
//                 href="/story" 
//                 className="hidden md:flex items-center gap-2 text-[13px] font-bold text-slate-500 hover:text-amber-600 transition-colors px-4 py-2 rounded-xl hover:bg-amber-50"
//               >
//                 ТҮҮХ НАМТАР
//               </Link>
...
import { ChevronRight, Fingerprint, ChevronDown, Plus, LogOut } from "lucide-react"; 
import Link from "next/link";
...
export default function LandingPage({ onLogout }) {
  ...
  // --- 1. ФУНКЦҮҮД (Scope-оос гадуур, хаанаас ч дуудаж болно) ---
  ...
  useEffect(() => {
    fetchProfiles(); // Анх ачаалахад дата татах
    
    // LocalStorage-оос нэр унших
  ...
  const sortedGenerations = Object.keys(groupedByGeneration).sort((a, b) => Number(a) - Number(b));
  const visibleGenerations = showAll ? sortedGenerations : sortedGenerations.slice(0, 4);
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 md:py-6">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-[8px] md:text-[9px] font-black text-amber-600 uppercase tracking-[0.2em] leading-none mb-1">Digital Archive</span>
                <h1 className="text-lg md:text-2xl font-black text-slate-900 leading-none tracking-tight">
                  Алтан Ургийн <span className="text-amber-500">Хэлхээ</span>
                </h1>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 md:gap-6">
              ...
              <div className="hidden lg:flex flex-col items-end border-r border-slate-200 pr-6">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Тавтай морил</span>
                <span className="text-[13px] font-black text-slate-700">Сайн байна уу, <span className="text-amber-600">{userName}</span>?</span>
              </div>

              <Link href="/story" className="hidden md:flex text-[11px] font-black text-slate-500 hover:text-amber-600 uppercase tracking-widest">Түүх намтар</Link>

              <button onClick={onLogout} className="group flex items-center gap-2 text-[10px] font-black text-slate-400 hover:text-red-500 uppercase tracking-wider">
                <LogOut size={16} className="group-hover:-translate-x-1 transition-transform" />
                <span className="hidden sm:inline">Гарах</span>
              </button>

              <button onClick={() => setIsRegisterOpen(true)} className="group flex items-center gap-2 px-3 py-1.5 md:px-5 md:py-2.5 bg-slate-900 text-white rounded-xl hover:bg-amber-500 transition-all shadow-lg shadow-slate-200">
                <Plus size={16} strokeWidth={3} />
                <span className="text-[10px] md:text-[12px] font-black uppercase tracking-widest">Нэмэх</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="flex-grow pt-24 md:pt-32">
        <main className="max-w-6xl mx-auto px-6 pb-12">
          ...
          <div className="lg:hidden mb-6 px-1">
            <h2 className="text-lg font-black text-slate-800">Сайн байна уу, <span className="text-amber-500">{userName}</span>? 👋</h2>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1">Өнөөдөр ургийн бичгээ баяжуулаарай.</p>
          </div>

          {loading ? (
            <div className="flex justify-center py-20"><div className="w-6 h-6 border-2 border-slate-200 border-t-amber-500 rounded-full animate-spin" /></div>
          ) : (
            <div className="space-y-8 md:space-y-12">
                ...
                <section key={gen} className="group">
                  <div className="flex items-center justify-between mb-4 px-1">
                    <div className="flex items-center gap-3">
                      <div className="bg-white border border-slate-200 rounded-lg px-3 py-1.5 shadow-sm">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mr-2">ҮЕ</span>
                        <span className="text-sm font-black text-slate-800">{gen}</span>
                      </div>
                      <span className="text-xs font-black text-amber-600">{groupedByGeneration[gen].length} гишүүн</span>
                    </div>
                    <Link href={`/generation/${gen}`} className="text-[10px] font-black text-slate-400 hover:text-amber-600 uppercase tracking-widest">Бүгдийг үзэх</Link>
                  </div>
                  ...
                  <div className="relative overflow-hidden">
                    <div className="flex overflow-x-auto pb-4 gap-4 no-scrollbar scroll-smooth snap-x">
                      {groupedByGeneration[gen].map((profile) => (
                        <div key={profile._id} className="flex-none snap-start">
                          <ProfileCard profile={profile} onDelete={handleDeleteFromState} onEdit={handleEdit} />
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {sortedGenerations.length > 4 && (
                <div className="flex justify-center pt-4">
                  <button onClick={() => setShowAll(!showAll)} className="group flex flex-col items-center gap-2">
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 group-hover:text-amber-600">{showAll ? "Хураах" : "Бүгдийг харах"}</span>
                    <div className="flex items-center justify-center w-10 h-10 rounded-full border border-slate-200 bg-white group-hover:border-amber-400 transition-all"><ChevronDown size={18} className={`text-slate-400 group-hover:text-amber-600 transition-transform ${showAll ? "rotate-180" : ""}`} /></div>
                  </button>
                </div>
              )}
            </div>

...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error("Имэйл эсвэл нууц үг буруу байна.");
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-[2.5rem] shadow-xl border border-white p-8 md:p-10">
          <div className="mb-8 text-center">
            ...
            <h2 className="text-3xl font-bold text-slate-900">Нэвтрэх</h2>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5 group">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Имэйл</label>
              <div className="relative group">
                ...
                <input type="email" name="email" placeholder="mail@example.com" value={formData.email} onChange={handleChange} required className="w-full pl-12 pr-4 py-3.5 bg-slate-50/50 border border-slate-100 rounded-2xl outline-none focus:bg-white focus:border-indigo-200 text-sm" />
              </div>
            </div>
            ...
            <div className="space-y-1.5 group">
              ...
              <div className="relative group">
                ...
                <input type={showPassword ? "text" : "password"} name="password" placeholder="••••••••" value={formData.password} onChange={handleChange} required className="w-full pl-12 pr-12 py-3.5 bg-slate-50/50 border border-slate-100 rounded-2xl outline-none focus:bg-white focus:border-indigo-200 text-sm" />
            ...
            <button type="submit" disabled={loading} className="w-full mt-4 bg-slate-900 hover:bg-black text-white font-bold py-4 rounded-2xl shadow-lg active:scale-[0.98] transition-all disabled:opacity-70 flex items-center justify-center gap-2">
              {loading ? <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <><span>Нэвтрэх</span><ChevronRight size={18} /></>}
            </button>
          </form>

          <div className="mt-8 pt-6 border-t border-slate-100 text-center text-sm font-medium">
            <p className="text-slate-400">Шинэ хэрэглэгч үү? <Link href="/signup" className="text-indigo-600 font-bold hover:underline">Бүртгүүлэх</Link></p>
          </div>
        </div>
      </div>
    </div>

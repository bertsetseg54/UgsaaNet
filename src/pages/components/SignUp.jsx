...
import {
  ...
  ChevronRight, Hash, Users, CheckCircle2, QrCode, X, Camera
} from "lucide-react";
// QR уншихад ашиглах сан: npm install html5-qrcode
...
export default function SignUp() {
  ...
  // QR Scanner-ийг ажиллуулах функц
  useEffect(() => {
    if (showScanner) {
      ...
      scanner.render((decodedText) => {
        ...
        // Уншиж чадаагүй үед алдааг энд барьж болно
      });
  ...
  const handleSubmit = async (e) => {
    ...
    if (familyMode === "create" && !formData.familyName) return setError("Ургийн нэрийг оруулна уу");
    if (familyMode === "join" && !familyId) return setError("Ургийн кодыг оруулна уу");
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-[#F8FAFC] px-4 py-8 selection:bg-indigo-100">
      ...
      {showScanner && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md">
          <div className="bg-white w-full max-w-sm rounded-[2rem] p-6 relative">
            <button onClick={() => setShowScanner(false)} className="absolute top-4 right-4 p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors">
              <X size={20} />
            </button>
            <h3 className="text-lg font-bold mb-4 text-center">QR код уншуулах</h3>
            <div id="reader" className="overflow-hidden rounded-2xl border-none"></div>
            <p className="mt-4 text-[11px] text-slate-400 text-center uppercase font-bold tracking-widest">
              Камераа кодонд ойртуулна уу
            </p>
          </div>
        </div>
      )}
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2.5rem] p-10 shadow-2xl max-w-sm w-full text-center animate-in zoom-in duration-300">
      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-[2.5rem] shadow-xl border border-white p-6 md:p-10 relative">
          ...
          {error && <div className="mb-6 p-4 rounded-2xl bg-red-50 border border-red-100 text-red-500 text-xs font-medium text-center animate-shake">{error}</div>}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button 
                      ...
                      className="p-4 bg-indigo-50 text-indigo-600 rounded-2xl hover:bg-indigo-100 transition-colors shadow-sm flex items-center justify-center shrink-0"
                    >
                      <QrCode size={20} />
                    </button>
                  </div>
              ...
              {/* Бусад талбарууд (Name, Email, Password) - Таны өмнөх код хэвээрээ */}
              <div className="space-y-1.5">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-indigo-500 transition-colors" size={18} />
                    <input type={showPassword ? "text" : "password"} name="confirmPassword" placeholder="••••" value={formData.confirmPassword} onChange={handleChange} required className="w-full pl-12 pr-12 py-3.5 bg-slate-50/50 border border-slate-100 rounded-2xl outline-none focus:bg-white focus:border-indigo-200 transition-all text-sm" />
                  </div>
                </div>
              </div>
            </div>

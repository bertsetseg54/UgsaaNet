...
import { 
  ArrowLeft, Crown, User, BookOpen, Edit3, Trash2, 
  CheckCircle2, Heart, Cross, Sparkles, ChevronUp
} from "lucide-react"; 
import RegisterForm from "../components/RegisterForm";
import Link from "next/link";
...
export default function PersonProfilePage() {
  ...
  // Дээшээх үеийн нэршлийг тодорхойлох функц
  const getAncestorLabel = (index, gender) => {
    if (index === 0) return gender === "female" ? "Ээж" : "Аав";
    if (index === 1) return gender === "female" ? "Эмээ" : "Өвөө";
    if (index === 2) return "Элэнц " + (gender === "female" ? "эмээ" : "өвөө");
    if (index === 3) return "Хуланц " + (gender === "female" ? "эмээ" : "өвөө");
    return `${index + 1} дахь үеийн дээдэс`;
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const found = allData.find(p => p._id === id);
        
        if (found) {
          setPerson(found);
          
          // Дээшээ бүх өвөг дээдсийг шат дараалан олох
          const chain = [];
          let currentParentId = found.parentId;
          while (currentParentId) {
            const p = allData.find(x => x._id === currentParentId);
            if (p) {
              chain.push(p);
          ...
          // Ах дүүс болон үр хүүхэд
          setSiblings(allData.filter(p => p.parentId === found.parentId && p._id !== found._id));
          setChildren(allData.filter(p => p.parentId === found._id));
          window.scrollTo({ top: 0, behavior: 'smooth' });
        }
      }
  ...
  const handleDelete = async () => {
    try {
      ...
      if (data.success) {
        setIsDeleteModalOpen(false);
        setSuccessModal({ open: true, message: "Амжилттай устгалаа", type: "delete" });
      }
  ...
  if (!person) return <div className="min-h-screen flex items-center justify-center"><p>Олдсонгүй.</p></div>;

  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col antialiased">
      <div className="max-w-2xl mx-auto w-full pt-1 px-3 flex-grow pb-10">
        
        {/* Буцах болон үйлдлүүд */}
        <div className="flex justify-between items-center mb-3">
          <Link href="/landingPage" className="text-slate-400 hover:text-slate-900 text-[10px] font-black uppercase tracking-widest flex items-center gap-1">
            <ArrowLeft size={14} /> БУЦАХ
          </Link>
          <div className="flex gap-2">
            <button onClick={() => setIsEditModalOpen(true)} className="p-2 bg-white border border-slate-100 rounded-lg text-slate-600 shadow-sm"><Edit3 size={14} /></button>
            <button onClick={() => setIsDeleteModalOpen(true)} className="p-2 bg-white border border-slate-100 rounded-lg text-red-400 shadow-sm"><Trash2 size={14} /></button>
          </div>
        </div>

        {/* Үндсэн мэдээлэл */}
        <div className={`bg-white border rounded-[2rem] overflow-hidden shadow-sm mb-1 ${Number(person.generation) === 1 ? 'border-amber-200 ring-2 ring-amber-50' : 'border-slate-50'}`}>
          <div className="p-4 flex flex-col md:flex-row gap-5 items-center md:items-start text-center md:text-left">
            <div className="shrink-0 relative">
              <div className="w-24 h-32 md:w-32 md:h-40 bg-slate-50 border-2 border-white rounded-2xl overflow-hidden shadow-inner">
                {(person.pic || person.imageUrl) ? (
                  <img src={person.pic || person.imageUrl} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-slate-200"><User size={40} /></div>
                )}
              </div>
              <div className="absolute -bottom-1 -right-1 bg-slate-900 text-white w-7 h-7 rounded-lg flex items-center justify-center border-2 border-white font-black text-[10px]">
                {person.generation}
              </div>
            </div>

            <div className="flex-1 w-full">
              <h1 className="text-2xl font-black text-slate-900 uppercase italic mb-1">{person.name}</h1>
              <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest bg-amber-50 inline-block px-2 py-0.5 rounded-md mb-4">{person.job || "Мэргэжилгүй"}</p>
              
              <div className="grid grid-cols-2 gap-y-3 gap-x-4 pt-3 border-t border-slate-50 text-left">
                <InfoItem label="Төрсөн он" value={person.birthyear} />
                <InfoItem label="Хүйс" value={person.gender === "male" ? "Эрэгтэй" : "Эмэгтэй"} />
                <InfoItem label={person.deathyear ? "Нас барсан" : "Хаяг"} value={person.deathyear || person.currentplace} highlight={!!person.deathyear} />
                <InfoItem label="Төрсөн нутаг" value={person.bornplace} />
              </div>
            </div>
          </div>
        ...
        {/* ХӨНДЛӨН ГҮЙДЭГ ХЭСГҮҮД */}
        <div className="flex flex-col gap-1">
          
          {/* 1. Өвөг дээдэс (Аав, Өвөө...) */}
          {ancestors.length > 0 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1 flex items-center gap-1">
                <ChevronUp size={12} /> Өвөг дээдэс
              </h3>
              <div className="flex gap-2 overflow-x-auto pb-2 px-1 snap-x scrollbar-hide">
                {ancestors.map((anc, index) => (
                  <Link key={anc._id} href={`/person/${anc._id}`} className="min-w-[140px] flex items-center gap-3 p-2 bg-white border border-slate-100 rounded-xl shadow-sm snap-start shrink-0 group hover:border-amber-400 transition-all">
                    <div className="w-9 h-9 rounded-lg bg-slate-50 overflow-hidden shrink-0 border border-slate-100">
                      {anc.pic ? <img src={anc.pic} className="w-full h-full object-cover" /> : <User size={14} className="m-auto mt-2 text-slate-300" />}
                    </div>
                    <div className="overflow-hidden">
                      <p className="text-[10px] font-black text-slate-800 uppercase truncate leading-none mb-1">{anc.name}</p>
                      <p className="text-[8px] text-amber-600 font-black uppercase tracking-tighter">{getAncestorLabel(index, anc.gender)}</p>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {/* 2. Ах дүүс */}
          <HorizontalSection title="Ах дүүс" count={siblings.length} data={siblings} />
          
          {/* 3. Үр хүүхэд */}
          <HorizontalSection title="Үр хүүхэд" count={children.length} data={children} />

        </div>

        {/* Намтар */}
        <div className="mt-3 p-4 bg-white border border-slate-50 rounded-[1.5rem]">
            <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                <BookOpen size={12} className="text-amber-500" /> Намтар түүх
            </h3>
            <p className="text-[12px] text-slate-600 leading-relaxed italic whitespace-pre-wrap">{person.about || "Мэдээлэл бөглөөгүй."}</p>
        </div>
      </div>

      {/* Footer */}
      <footer className="py-4 text-center">
        <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest">© 2026 UGSAANET</p>
      </footer>

      {/* Modals */}
      {isEditModalOpen && <ModalConfirm title="Мэдээллийг засах уу?" onConfirm={() => {setIsEditModalOpen(false); setIsEditOpen(true);}} onCancel={() => setIsEditModalOpen(false)} icon={<Sparkles size={24} className="text-amber-500" />} />}
          {isEditModalOpen && (
      <ModalConfirm 
        onConfirm={() => {setIsEditModalOpen(false); setIsEditOpen(true);}} 
        onCancel={() => setIsEditModalOpen(false)} 
        icon={<div className="bg-amber-50 p-2 rounded-xl flex gap-1 items-center justify-center"><Sparkles size={20} className="text-amber-600" /><span className="text-slate-950 font-[1000] uppercase text-[12px] tracking-tighter">Мэдээллийг засах уу?</span></div>}
        className="p-4 max-w-[260px] rounded-[1.5rem]" // Модаль доторх зайг шахах класс
      />
    )}
    ...
    {isDeleteModalOpen && (
      <ModalConfirm 
        ...
        danger 
        icon={
        <div className="bg-red-50 p-2 rounded-xl flex gap-1 items-center justify-center">
          <Trash2 size={20} className="text-red-600" />
          <span className="text-slate-950 font-[1000] uppercase text-[12px] tracking-tighter">Мэдээллийг устгах уу?</span>
        </div>
        }
        className="p-4 max-w-[260px] rounded-[1.5rem]"
      />
    )}
      {isEditOpen && <RegisterForm isOpen={isEditOpen} setIsOpen={setIsEditOpen} editData={person} onUpdate={fetchData} />}
    </div>
  );
}

// Хөндлөн жагсаалтын компонент
function HorizontalSection({ title, count, data }) {
    if (data.length === 0) return null;
    return (
      <div>
        <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1 flex justify-between">
          <span>{title}</span>
          <span className="opacity-40">{count}</span>
        </h3>
        <div className="flex gap-2 overflow-x-auto pb-2 px-1 snap-x scrollbar-hide">
          {data.map((item) => (
            <Link key={item._id} href={`/person/${item._id}`} className="min-w-[120px] flex items-center gap-2 p-2 bg-white border border-slate-100 rounded-xl shadow-sm snap-start shrink-0 hover:border-amber-200 transition-all">
              <div className="w-8 h-8 rounded-lg bg-slate-50 overflow-hidden shrink-0">
                {item.pic ? <img src={item.pic} className="w-full h-full object-cover" /> : <User size={12} className="m-auto mt-2 text-slate-300" />}
              </div>
              <span className="text-[10px] font-bold text-slate-700 uppercase truncate">{item.name}</span>
            </Link>
          ))}
        </div>
      </div>
...
function InfoItem({ label, value, highlight }) {
  return (
    <div>
      <label className={`text-[8px] font-black uppercase block mb-0.5 ${highlight ? 'text-red-400' : 'text-slate-400'}`}>{label}</label>
      <p className={`text-[11px] font-bold truncate ${highlight ? 'text-red-700' : 'text-slate-800'}`}>{value || "---"}</p>
    </div>
  );
}

function ModalConfirm({ title, onConfirm, onCancel, icon, danger }) {
    return (
      <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
        <div className="bg-white rounded-[2rem] p-6 max-w-xs w-full text-center shadow-xl">
          <div className="mb-3">{icon}</div>
          <h3 className={`text-md font-black uppercase mb-5 ${danger ? 'text-red-600' : 'text-slate-900'}`}>{title}</h3>
          <div className="flex flex-col gap-2">
            <button onClick={onConfirm} className={`w-full py-3 rounded-xl text-[10px] font-black uppercase text-white ${danger ? 'bg-red-500' : 'bg-slate-900'}`}>Тийм</button>
            <button onClick={onCancel} className="w-full py-3 text-slate-400 text-[10px] font-black uppercase">Болих</button>
          </div>
        </div>
      </div>
    );
